...
boxplot(usa$Rating, france$Rating, canada$Rating, names=strTop3[1:3], ylab="Rating", main="Rating Distributions by Country", col = c("red", "green", "Gold"))

```


4)
We wanted to see which bean origins had the best average ratings.
```{r}
stuff <- aggregate(Rating ~ Specific.Bean.Origin, data = choco, mean)
head(stuff[order(-stuff$Rating),])
```
